...
/**
 * ReportDataView — visualización de los reportes del Agente Avanzado.
 *
 ...
 *  - Categorías                      → barras / rectángulos (treemap)
 *  - Demás casos                     → tabla limpia con tabular-nums
 *
 * Extiende a <AgentDataView> (chat) con: treemap, etiquetas de valor/porcentaje,
 * gráfica fija (lockViz), tabla junto a la gráfica y drill-down al hacer clic.
 */
...
interface ReportDataViewProps {
    ...
    onDrill?: (value: string) => void;  // clic en una categoría → drill-down
}
...
function formatCell(key: string, value: any): string {
    ...
    if (typeof value === 'number') {
        return formatNumber(value, { currency: isCurrencyKey(key), percent: isPercentKey(key) });
    }
...
function detectAutoViz(data: Record<string, any>[], suggested?: string): Exclude<Viz, 'auto'> {
    ...
    if (data.length >= 2 && data.length <= 20 && numKeys.length >= 1 && !hasTemporal) {
        return data.length <= 8 ? 'bar' : 'table';
    }
            ...
            <ResponsiveContainer width="100%" height="100%">
                {type === 'line' ? (
                    ...
                    <AreaChart {...commonChartProps}>
                        <defs>
                            {numKeys.map((_, i) => (
                                <linearGradient key={i} id={`report-area-grad-${i}`} x1="0" y1="0" x2="0" y2="1">
...
export function ReportDataView({ data, suggestedViz, question, lockViz, showValues, showPercent, alsoTable, onDrill }: ReportDataViewProps) {
    ...
    if (data.length >= 2 && data.length <= 50 && numKeys.length > 0) {
        ...
        if (data.length <= 7) availableVizs.push({ id: 'pie', label: 'Pastel', icon: PieIcon });
    ...
    const handleExport = () => {
        ...
        const filename = question ? question.slice(0, 30).replace(/[^a-z0-9]+/gi, '_') : 'reporte_nexus';
        writeFile(wb, `${filename}.xlsx`);
    };

    return (
        <div className="space-y-2">
            {/* Selector de vista + exportación */}
